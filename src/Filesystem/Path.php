...
class App_Filesystem_Path {
    ...
    public static function check($path) {
        if (strpos($path, '..') !== false) {
            // Don't translate
            throw new Exception('Check Use of relative paths not permitted', 20);
        }

        $path = self::clean($path);

        if ((PATH_PROJECT != '') && strpos($path, self::clean(PATH_PROJECT)) !== 0) {
            throw new Exception('Check Snooping out of bounds @ ' . $path, 20);
        }
