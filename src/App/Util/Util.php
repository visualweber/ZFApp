...
class App_Util_Util {
     ...
     * @return \DateTime
     */
    public static function parseDate($date = '', $datas = array()) {
        $day = rand(1, 28);
        $month = $year = '';
        $startTime = $endTime = $datas['startTime'] = $datas['endTime'] = '';
        if (substr_count(mb_strtolower($date), 'đến')):
            $timeperiods = explode('đến', mb_strtolower($date));
            $timeperiods = array_filter(array_map('trim', $timeperiods));

            //Lay datetime kieu number, neu date la string se xet NULL
            $filter_date = preg_replace('/[^0-9^\-]+/', ' ', reset($timeperiods));
            if (trim($filter_date)):
                $renew = preg_replace('/[^0-9^\-]+/', '-', trim($filter_date));
                $explode = explode('-', trim($renew));
                ...
                if (!empty($explode)):
                    ...
                    elseif (count($explode) == 3):
                        $day = reset($explode);
                        $month = preg_replace('/[^0-9]/', '', $explode[1]);
                    else:
                        $month = preg_replace('/[^0-9]/', '', reset($explode));
                    endif;
                endif;
                $year = preg_replace('/[^0-9]/', '', end($explode));
                if ((int) $day < 32 and (int) $month < 13 and strlen($year) == 4):
                    $startTime = (int) $day . '-' . (int) $month . '-' . (int) $year;
                endif;
                $day = $month = $year = '';
                if ($startTime):
                    $datas['startTime'] = new \DateTime($startTime);
                endif;
            endif;

            //Lay datetime kieu number, neu date la string se xet NULL
            $filter_date = preg_replace('/[^0-9^\-]+/', ' ', end($timeperiods));
            if (trim($filter_date)):
                $renew = preg_replace('/[^0-9^\-]+/', '-', trim($filter_date));
                $explode = explode('-', trim($renew));
                $explode = array_filter($explode);
                ...
                $year = preg_replace('/[^0-9]/', '', end($explode));
                if ((int) $day < 32 and (int) $month < 13 and strlen($year) == 4):
                    $endTime = (int) $day . '-' . (int) $month . '-' . (int) $year;
                endif;
                $day = $month = $year = '';
                if ($endTime):
            ...
            $exp_timeend = $time = '';
            if (count($explode) == 3):
                $day = preg_replace('/[^0-9]/', '', reset($explode));
                $month = preg_replace('/[^0-9]/', '', $explode[1]);
                $year = preg_replace('/[^0-9]/', '', end($explode));
            ...
            elseif (count($explode) == 1):
                $year = preg_replace('/[^0-9]/', '', reset($explode));
            endif;
            ...
            if ($time):
                $exp_timeend = new \DateTime($time);
            endif;
            $datas['startTime'] = $datas['endTime'] = $exp_timeend;
        endif;
        return $datas;
    }
