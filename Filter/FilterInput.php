...
class App_Filter_FilterInput
{
     ...
     * 		<pre>  $filter = & App_Filter_FilterInput::getInstance();</pre>
     *
     * @static
     * @param	array	$tagsArray	list of user-defined tags
     * @param	array	$attrArray	list of user-defined attributes
     ...
     * @param	int		$attrMethod	WhiteList method = 0, BlackList method = 1
     * @param	int		$xssAuto	Only auto clean essentials = 0, Allow clean blacklisted tags/attr = 1
     * @return	object	The App_Filter_FilterInput object.
     */
    public function & getInstance($tagsArray = array(), $attrArray = array(), $tagsMethod = 0, $attrMethod = 0, $xssAuto = 1)
    {
        ...
        if (empty ($instances[$sig])) {
            $instances[$sig] = new App_Filter_FilterInput($tagsArray, $attrArray, $tagsMethod, $attrMethod, $xssAuto);
        }
    ...
    public function clean($source, $type='string')
    {
        ...
        switch (strtoupper($type))
        {
            ...
            case 'STRING' :
                ...
                if(isset($this) && is_a( $this, 'App_Filter_FilterInput' )) {
                    $filter =& $this;
                } else {
                    $filter =& App_Filter_FilterInput::getInstance();
                }
            ...
            default :
                // Check for static usage and assign $filter the proper variable
                if(is_object($this) && get_class($this) == 'App_Filter_FilterInput') {
                    $filter =& $this;
                } else {
                    $filter =& App_Filter_FilterInput::getInstance();
                }
    ...
    public function _cleanAttributes($attrSet)
    {
        ...
        for ($i = 0; $i < count($attrSet); $i ++)
        {
            ...
            if (App_Filter_FilterInput::checkAttribute($attrSubSet)) {
                continue;
            }
